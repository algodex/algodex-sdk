 ...
 *
 * Place a buy order into the Algodex Orderbook. This is referred to as a "Maker" order which is
...
async function makePlaceAlgoTxns(
  ...
  console.log('Make Palce Algo Txns', order, optIn);
  ...
  if (typeof order?.wallet?.assets === 'undefined') {
    ...
    accountInfo = await order.client.accountInformation(order.address).do();
  } else {
    accountInfo = order.wallet;
  }

  const _optIn = await getOptedIn(order.client, accountInfo, order.asset.id);
  ...
  if (!_exists) {
    ...
    const _appArgs = [
      enc.encode('open'),
      // enc.encode(order.contract.entry.slice(59)),Note: when going through taker with Remainder therer is no address attached to entry
