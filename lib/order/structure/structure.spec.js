...
describe('structure', () => {
  ...
  it('should structure a single transaction when placeAlgo escrow already exists', async () => {
    ...
    const structuredOrder = await structure(config.api, _order);
  ...
  it('should structure and send taker split order buy transactions', async () => {
    ...
    const outerTxns = await structure(config.api, {..._order, appId: appIdForAssetEscrow});
    signedTxns = await sdkSigner(outerTxns.contract.txns, config.api.wallet.connector.sk);
    signedTxns.forEach((inner) => {
      expect(typeof inner.signedTxn.txID).toEqual('string');
    });
    if (process.env.TEST_ENV === 'integration') {
