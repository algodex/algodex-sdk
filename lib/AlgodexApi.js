...
const _sendTransactions = async (client, signedTransactions) => {
  // const sendAndWait = async (txnBlobs) => {
  //   const { txId } = await client.sendRawTransaction(txnBlobs).do();
  //   await algosdk.waitForConfirmation(client, txId, 10);
  // };
  // await Promise.all(
  //   signedTransactions.map((group) => {
  //     logger.debug(`Sending ${group.length} Group Transactions`);
  //     const rawTxns = group.map((txn) => txn.blob);
  //     return sendAndWait(rawTxns);

  //     // client.sendRawTransaction(rawTxns).do();
  //     // logger.debug(`Waiting for confirmation for ${txId}`);
  //     // await algosdk.waitForConfirmation(client, txId, 10);
  //     logger.debug(`Confirmed ${txId}`);
  //   })
  // );
