...
Object.assign(AlgodexClient.prototype, {
  ...
  async fetchAssetChart(id, interval) {
    ...
    const res = await this.get(
      `/api/v2/trades/charts/asset/${id}/period/${interval}`  
  ...
  async fetchAssetOrders(id) {
    ...
    const url = `/api/v2/orders/asset/${id}`;
  ...
  async fetchAssetTradeHistory(id) {
    ...
    const res = await this.get(
        `/api/v2/trades/history/asset/${id}`,
  ...
  async fetchWalletTradeHistory(address, includeAssetInfo = true) {
    ...
    const res = await this.get(
     `/api/v2/trades/history/wallet/${address}`
  ...
  async fetchAssets() {
    ...
    } = await this.get(`http://localhost:3000/api/v2/assets/all`); //assets.php
    return data;
  },
  ...
  async fetchAssetPrice(id) {
    ...
    let res = {};
    try {
      res = await this.get(`${this.baseUrl}/assets.php?id=${id}`);
    } catch (error) {
      return {};
    }
    const {
