 ...
 * @param {Object} waleltObj WalletObject of User
 * @param {Object} order ExecutableOrder
 *
 * @return {Object} returns a compiled executable with relevant splitOrder amounts and totals
 * single transList
 ...
 *
 * @param {Object} compileObj Compiled executable object
 * @param {<TakerInformation>} takerOrderBalance Object containing the relevant balance information
 *
 * @return {Object} returns a compiled executable with relevant splitOrder amounts and totals
 * single transList
 * @memberOf module:order/structures
 ...
 * @memberOf module:order/structures
 */
function withQueuedOrder(order, queuedOrder, takerOrderBalance) {
  if (takerOrderBalance.isExecuteASA) {
    return {
      ...order,
 ...
 * @param {<TakerInformation>} takerOrderBalance An object containing relevant information related to the current balance. Used to decide when to stop executing on queuedOrders
    ...
    for (let jj = 0; jj < splitTimes; jj++) {
      ...
      const singleOrderTransList =
                !queuedOrder.isASAEscrow ?
                    await withExecuteAlgoTxns(withQueuedOrder(compiledEscrow, queuedOrder, updatedTakerOrderBalance), queuedOrder?.shouldClose) :
                    await withExecuteAssetTxns(withTakerBalanceAmounts(withUserWallet(order.wallet, compiledEscrow), updatedTakerOrderBalance), queuedOrder?.shouldClose);
