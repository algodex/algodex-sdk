 ...
 * @param {boolean} [etags] Flag to enable Etag on GET Requests (optional)
 * @param {integer} [apiVersion] 1 or 2 (optional)
 * @param {string} [v1Url] Set this for any APIs that need to use v1 (optional)
 * @extends HTTPClient
 * @ignore
 * @constructor
 */
function AlgodexClient(baseUrl, etags=true, apiVersion=1, v1Url=undefined) {
  // Apply client
...
Object.assign(AlgodexClient.prototype, {
   ...
   *
   * @param {object} data
   * @param {object} assetSearchData // Used as backup for matching asset names
   * @return {null|*}
   */
  mapOpenOrdersData(data, assetSearchData=null) {
    if (
      !data ||
      !data.buyASAOrdersInEscrow ||
      !data.sellASAOrdersInEscrow
      // FIXME - removed null check for allAssets due to serverside issue
    ) {
      return null;
    }

    const getAssetMap = (assetSearchData) => {
      if (assetSearchData?.assets) {
        return assetSearchData.assets.reduce((map, asset) => {
          const unitName = asset.unitName;
          map.set(asset.assetId, unitName);
          return map;
        }, new Map());
      }
      return null;
    ...
    const assetToNameMap = getAssetMap(assetSearchData);
    ...
    const buyOrders = buyOrdersData.map((order) => {
      ...
      return {
        ...
        pair: assetsInfo[assetId] ? `${assetsInfo[assetId]?.params['unit-name']}/ALGO` :
          assetToNameMap?.get(assetId)+'/ALGO',
        type: 'BUY',
        status: 'OPEN',
  ...
  async fetchAssetChart(id, interval) {
    ...
    const res = await this.get(
      this.apiVersion === 2 ? `${this.baseUrl}/trades/charts/asset/${id}/period/${interval}` :
  ...
  async fetchAssetOrders(id) {
    ...
    // const baseUrl = this.v1Url ? this.v1Url : this.baseUrl;

    // const url = `${baseUrl}/orders.php?assetId=${id}`; // temporarily do not use v2
    const url = this.apiVersion === 2 ? `${this.baseUrl}/orders/asset/${id}` :
       `${this.baseUrl}/orders.php?assetId=${id}`;
  ...
  async fetchWalletOrders(address, includeAssetInfo = true) {
    ...
    // );
    console.log('in fetchWalletOrders');
    const res = await this.get(
      this.apiVersion === 2 ? `${this.baseUrl}/orders/wallet/${address}` :
  ...
  async fetchAssetTradeHistory(id) {
    ...
    const res = await this.get(
      this.apiVersion === 2 ? `${this.baseUrl}/trades/history/asset/${id}` :
  ...
  async fetchWalletTradeHistory(address, includeAssetInfo = true) {
    ...
    const res = await this.get(
      this.apiVersion === 2 ? `${this.baseUrl}/trades/history/wallet/${address}` :
  ...
  async fetchWalletAssets(address) {
    ...
    const res = await this.get(
      this.apiVersion === 2 ? `${this.baseUrl}/wallet/assets/${address}` :
  ...
  async searchAssets(query) {
    ...
    // FIXME
    // const url = this.v1Url ? this.v1Url : this.baseUrl;
    const v2Url = query && query.length > 0 ? `${this.baseUrl}/assets/search/${query}` :
      `${this.baseUrl}/assets/searchall`;

    const res = await this.get(
      this.apiVersion === 2 ? v2Url :
       `${this.baseUrl}/asset_search.php?query=${query}`,
        // `${url}/asset_search.php?query=${query}`,
    );
   ...
   */
  async fetchAssets() {
    logger.debug(`fetchAssets() api version: ${this.apiVersion}`);
    const {
      data: {data}, // FIXME for API below since it is used for prefetching
    } = await this.get(this.apiVersion === 2 ? `${this.baseUrl}/assets/all` :
      `${this.baseUrl}/assets.php`);
    return data;
  },
  ...
  async fetchAssetPrice(id) {
    ...
    let res = {};
    try {
      res = await this.get(this.apiVersion === 2 ? `${this.baseUrl}/assets/${id}` :
        `${this.baseUrl}/assets.php?id=${id}`);
    } catch (error) {
      return {};
    }
    const {
    ...
    return typeof data[0] !== 'undefined' ? data[0] : {};
  },
});
