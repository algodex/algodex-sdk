...
async function makePlaceAssetTxns(
  ...
  if (!exists) {
    ...
    const _appArgs = [
      ...
      new Uint8Array([order.version]),
		...
		const _outerTxns = [{
			// Payment Transaction
			unsignedTxn: await makePaymentTxn({...order, "contract":{"amount": 50000}}, closeRemainderTo,  note), 
  ...
  if (!makerAlreadyOptedIntoASA) {
    ...
    // OptIn Transaction
    _outerTxns.push({
      unsignedTxn: await makeAssetOptInTxn({...order, "contract":{"from": lsig.address()}}),
      senderAcct: lsig.address(),
    });
  }
