 ...
 * @type {string}
 */
const ALGOD_SERVER = 'http:/ec2-3-18-80-65.us-east-2.compute.amazonaws.com';
/**
 * @deprecated
 * @type {string}
 */
const ALGOD_TOKEN =
  '11e4dcfb445a8c7e8380848747f18afcd5d84ccb395e003e5b72127ca5e9a259'; // { 'X-API-Key': 'VELyABA1dGqGbAVktbew4oACvp0c0298gMgYtYIb' }
/**
 * @deprecated
 * @type {string}
 */
...
async function getAccountInfo(addr) {
  try {
    ...
    logger.error(undefined, e.message);
    throw new Error(`getAccountInfo failed: ${e.message}`);
  }
}
  ...
  for (const group in groupsOfTransactions) {
    if (group !== 'prototype') {
      const { txId } = await client
        .sendRawTransaction(groupsOfTransactions[group])
        .do();
...
async function deleteApplication(client, sender, appId) {
  ...
  const params = await getTransactionParams(client);
  ...
  try {
    ...
    logger.error({ fn: 'deleteApplication' }, e.message);
  }
  // display results
  await algosdk.waitForConfirmation(client, txId, 10);
...
async function closeAccount(client, fromAccount, toAccount) {
  logger.debug(
    'teal.closeAccount - checking account info for: ' + fromAccount.addr
  );
  ...
  if (
    ...
    for (let i = 0; i < fromAccountInfo['assets'].length; i++) {
      ...
      const txn = await txns.makeAssetTransferTxn(
        client,
        fromAccount,
        toAccount,
        0,
