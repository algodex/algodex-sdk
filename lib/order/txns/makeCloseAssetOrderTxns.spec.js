...
describe('makeCloseAsset', ()=>{
  ...
  it('should have a valid client', async ()=>{
    await expect(makeCloseAssetOrderTxns({})).rejects.toBeInstanceOf(AlgodError);
  });

  it('should have a valid application index', async ()=>{
    await expect(makeCloseAssetOrderTxns({client: config.client, appId: ()=>{}})).rejects.toBeInstanceOf(TypeError);
  });

  // it('should have a valid contract entry', async ()=>{
  //   await expect(makeCloseAssetOrderTxns({client: config.client, appId: 123456, contract: {entry: 1234}})).rejects.toBeInstanceOf(TypeError);
  // });
  ...
  orders.map((o, idx)=>{
    ...
    let signedTxns;
    it(`should create CloseAssetOrderTxns ${idx}`, async ()=>{
      ...
      const outerTxns = await makeCloseAssetOrderTxns(_order, exist, optIn, undefined, undefined);
