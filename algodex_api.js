...
if (MyAlgo != null) {
  ...
  // logger.log("printing my algo wallet");
  // logger.log(myAlgoWallet)
}
...
const AlgodexApi = {
  ...
  initSmartContracts: function (environment) {
    ...
    //logger.log("ALGO APP ID:", ALGO_ESCROW_ORDER_BOOK_ID)
    //logger.log("ASA APP ID:", ASA_ESCROW_ORDER_BOOK_ID)

    dexInternal.initSmartContracts(
      ALGO_ESCROW_ORDER_BOOK_ID,
      ASA_ESCROW_ORDER_BOOK_ID
    )
    //logger.log({ALGO_ESCROW_ORDER_BOOK_ID, ASA_ESCROW_ORDER_BOOK_ID});
  },
  ...
  getMinWalletBalance: async function (accountInfo) {
    ...
    logger.log("in getMinWalletBalance. Checking: " + accountInfo.address)
    ...
    minBalance +=
      (25000 + 25000) * accountInfo["apps-total-schema"]["num-byte-slice"] // Total Bytes
    minBalance += accountInfo["assets"].length * 100000
    minBalance += 1000000

    logger.log({ minBalance })
  ...
  initAlgodClient: function (environment) {
    ...
    //logger.log({server: algodServer, token, port});
  ...
  createOrderBookEntryObj: function createOrderBookEntryObj(
    blockChainOrderVal,
    price,
    n,
    d,
  ...
  getCutOrderTimes: (queuedOrder) => {
    logger.log("in getCutOrderTimes: ", JSON.stringify(queuedOrder))
    let cutOrderAmount = null,
      splitTimes = null
  ...
  executeOrder: async function executeOrder(
    algodClient,
    isSellingASA,
    assetId,
    userWalletAddr,
    ...
    logger.log("in executeOrder")

    let queuedOrders = dexInternal.getQueuedTakerOrders(
      userWalletAddr,
      isSellingASA,
      allOrderBookOrders
    )
    ...
    logger.log("herezz56")
    logger.log({ execAccountInfo })

    let takerMinBalance = 0

    takerMinBalance += 100000 * execAccountInfo["created-apps"].length // Apps
    takerMinBalance +=
      (25000 + 3500) * execAccountInfo["apps-total-schema"]["num-uint"] // Total Ints
    takerMinBalance +=
      (25000 + 25000) * execAccountInfo["apps-total-schema"]["num-byte-slice"] // Total Bytes
    takerMinBalance += execAccountInfo["assets"].length * 100000
    takerMinBalance += 1000000

    logger.log({ min_bal: takerMinBalance })

    let walletAssetAmount = 0
    const walletAlgoAmount =
      execAccountInfo["amount"] - takerMinBalance - 0.004 * 1000000
    if (walletAlgoAmount <= 0) {
      logger.log("not enough to trade!! returning early")
      return
    }

    if (
      execAccountInfo != null &&
      execAccountInfo["assets"] != null &&
      ...
      for (let i = 0; i < execAccountInfo["assets"].length; i++) {
        ...
        if (asset["asset-id"] == assetId) {
          ...
          break
          //logger.log("execAccountInfo: " + execAccountInfo);
        }
      }
    }
    ...
    logger.log("initial taker orderbalance: ", this.dumpVar(takerOrderBalance))

    //let walletBalance = 10; // wallet balance
    //let walletASABalance = 15;
    if (queuedOrders == null && !includeMaker) {
      logger.log("null queued orders, returning early")
      return
    }
    ...
    for (let i = 0; i < queuedOrders.length; i++) {
      ...
      if (isSellingASA && parseFloat(takerOrderBalance["asaBalance"]) <= 0) {
        logger.log("breaking due to 0 asaBalance balance!")
        break
      }
      if (!isSellingASA && parseFloat(takerOrderBalance["algoBalance"]) <= 0) {
        logger.log("breaking due to 0 algoBalance balance!")
        break
      }
      ...
      logger.log("cutOrder, splitTimes: ", { cutOrder, splitTimes })
      ...
      for (let jj = 0; jj < splitTimes; jj++) {
        ...
        let singleOrderTransList =
          await dexInternal.getExecuteOrderTransactionsAsTakerFromOrderEntry(
            algodClient,
            queuedOrder,
            takerOrderBalance,
            params
    ...
    let makerTxns = null
    logger.log("here55999a ", { lastExecutedPrice, limitPrice })
    if (includeMaker) {
      const numAndDenom =
        lastExecutedPrice != -1
          ? this.getNumeratorAndDenominatorFromPrice(lastExecutedPrice)
      ...
      logger.log("includeMaker is true")
      if (isSellingASA && leftoverASABalance > 0) {
        logger.log("leftover ASA balance is: " + leftoverASABalance)

        makerTxns = await this.getPlaceASAToSellASAOrderIntoOrderbook(
          algodClient,
          userWalletAddr,
          numAndDenom.n,
          numAndDenom.d,
        ...
        logger.log("leftover Algo balance is: " + leftoverASABalance)

        makerTxns = await this.getPlaceAlgosToBuyASAOrderIntoOrderbook(
          algodClient,
          userWalletAddr,
          numAndDenom.n,
          numAndDenom.d,
    ...
    if (allTransList == null || allTransList.length == 0) {
      logger.log("no transactions, returning early")
    }
    ...
    logger.log("going to wait for confirmations")

    let waitConfirmedPromises = []

    for (let i = 0; i < sentTxns.length; i++) {
      logger.log("creating promise to wait for: " + sentTxns[i])
      const confirmPromise = this.waitForConfirmation(algodClient, sentTxns[i])
      waitConfirmedPromises.push(confirmPromise)
    }

    logger.log("final9 trans are: ")
    // logger.log(alTransList);
    // logger.log(transNeededUserSigList);

    logger.log("going to send all ")
    ...
    logger.log("trans results after confirmed are: ")
    logger.log(transResults)

    // await this.waitForConfirmation(algodClient, txn.txId);
    return
  },

  closeOrderFromOrderBookEntry: async function closeOrderFromOrderBookEntry(
    algodClient,
    escrowAccountAddr,
    creatorAddr,
    orderBookEntry,
    ...
    logger.log("closing order from order book entry!")
    logger.log(
    ...
    logger.log("args length: " + appArgs.length)
    ...
    let escrowSource = this.buildDelegateTemplateFromArgs(
      min,
      assetid,
      n,
      d,
    ...
    let lsig = await dexInternal.getLsigFromProgramSource(
      algosdk,
      algodClient,
      escrowSource,
      constants.DEBUG_SMART_CONTRACT_SOURCE
    )
    logger.log("lsig is: " + lsig.address())
    if (assetId == null) {
      logger.log("closing order")
      await dexInternal.closeOrder(
        algodClient,
        escrowAccountAddr,
        creatorAddr,
        ALGO_ESCROW_ORDER_BOOK_ID,
      ...
      await dexInternal.closeASAOrder(
        algodClient,
        escrowAccountAddr,
        creatorAddr,
        ASA_ESCROW_ORDER_BOOK_ID,
  ...
  signAndSendTransactions: async function signAndSendTransactions(
    algodClient,
    outerTxns
  ) {
    logger.log("inside signAndSend transactions")
    let txnsForSig = []
    let txns = []
    ...
    logger.log("printing transaction debug")
  ...
  generateOrder: function (
    ...
    return dexInternal.generateOrder(
      makerWalletAddr,
      n,
      d,
      min,
  ...
  getPlaceAlgosToBuyASAOrderIntoOrderbook:
    async function getPlaceAlgosToBuyASAOrderIntoOrderbook(
      ...
      logger.log(
        "placeAlgosToBuyASAOrderIntoOrderbook makerWalletAddr, n, d, min, assetId",
        makerWalletAddr,
        n,
        d,
        min,
      ...
      let program = this.buildDelegateTemplateFromArgs(
        min,
        assetId,
        n,
        d,
      ...
      let lsig = await this.getLsigFromProgramSource(
        algosdk,
        algodClient,
        program,
        constants.DEBUG_SMART_CONTRACT_SOURCE
      )
      let generatedOrderEntry = dexInternal.generateOrder(
        makerWalletAddr,
        n,
        d,
        min,
      ...
      logger.log("address is: " + lsig.address())
      logger.log("here111 generatedOrderEntry " + generatedOrderEntry)
      ...
      if (
        makerAccountInfo != null &&
        makerAccountInfo["assets"] != null &&
      ...
      if (
        ...
        escrowAccountInfo["apps-local-state"] != null &&
      ...
      logger.log({ makerAlreadyOptedIntoASA })
      logger.log({ alreadyOptedIntoOrderbook })
      ...
      logger.log("alreadyOptedIn: " + alreadyOptedIntoOrderbook)
      logger.log("acct info:" + JSON.stringify(escrowAccountInfo))

      let params = await algodClient.getTransactionParams().do()
      logger.log("sending trans to: " + lsig.address())
      let txn = {
        ...params,
        type: "pay",
      ...
      logger.log("here3 calling app from logic sig to open order")
      let appArgs = []
      var enc = new TextEncoder()
      appArgs.push(enc.encode("open"))
      //logger.log("before slice: " + generatedOrderEntry);
      logger.log(generatedOrderEntry.slice(59))
      //logger.log("after slice: " + generatedOrderEntry.slice(59));
      ...
      appArgs.push(arr)
      logger.log("app args 2: " + arr)
      //logger.log("owners bit addr: " + ownersBitAddr);
      //logger.log("herezzz_888");
      logger.log(appArgs.length)
      let logSigTrans = null

      if (!alreadyOptedIntoOrderbook) {
        logSigTrans = await dexInternal.createTransactionFromLogicSig(
          algodClient,
          lsig,
          ALGO_ESCROW_ORDER_BOOK_ID,
          appArgs,
  ...
  getPlaceASAToSellASAOrderIntoOrderbook:
    async function getPlaceASAToSellASAOrderIntoOrderbook(
      ...
      logger.log("checking assetId type")
      ...
      if (
        accountInfo != null &&
        accountInfo["apps-local-state"] != null &&
        accountInfo["apps-local-state"].length > 0 &&
        accountInfo["apps-local-state"][0].id == ASA_ESCROW_ORDER_BOOK_ID
      ) {
        alreadyOptedIn = true
      }
      logger.log("alreadyOptedIn: " + alreadyOptedIn)
      logger.log("acct info:" + JSON.stringify(accountInfo))
      ...
      logger.log("herez88888 ", this.dumpVar(assetSendTrans))
      ...
      logger.log("typeof: " + typeof payTxn.txId)
      logger.log("the val: " + payTxn.txId)
      ...
      let logSigTrans = await dexInternal.createTransactionFromLogicSig(
        algodClient,
        lsig,
        ASA_ESCROW_ORDER_BOOK_ID,
        appArgs,
  ...
  buildDelegateTemplateFromArgs: function buildDelegateTemplateFromArgs(
    min,
    assetid,
    N,
    D,
    ...
    return dexInternal.buildDelegateTemplateFromArgs(
      min,
      assetid,
      N,
      D,
  ...
  getLsigFromProgramSource: async function getLsigFromProgramSource(
    algosdk,
    algodClient,
    program,
    logProgramSource
  ) {
    return await dexInternal.getLsigFromProgramSource(
      algosdk,
      algodClient,
      program,
      logProgramSource
