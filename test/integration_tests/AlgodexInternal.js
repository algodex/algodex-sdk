...
const InternalTests = {
  createTransactionFromLsig: async function(config, price, isEscrow, appOptIn) {
    ...
    const generatedOrderEntry = algodexApi.generateOrder(config.creatorAccount.addr, 1, 2000, 0, config.assetId, true);
  ...
  formatMetaData: async function(config, orderType, currency) {
    algodexApi.initSmartContracts('test');
    const accountInfo = await algodexApi.getAccountInfo(config.creatorAccount.addr);
    ...
    try {
      const payTxn = await transactionGenerator.getPlaceASAEscrowOrderTxns(config.client, config.creatorAccount, 1, 2, config.assetId, 10, true);


      const formattedTxn = internalApi.formatTransactionsWithMetadata(payTxn, config.creatorAccount, noteMetadata, orderType, currency);
  ...
  closeOrder: async function(client, config) {
    ...
    const generatedOrderEntry = algodexApi.generateOrder(creatorAccount.addr, 1, 2000, 0, assetId, false);
    const appId = generatedOrderEntry.split('-')[generatedOrderEntry.split('-').length - 1];
