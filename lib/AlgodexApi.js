...
const TinymanClient = require('./http/clients/TinymanClient');
const {groupBy} = require('./functions/base');
const structureRefreshed = require('./order/structures/structureRefreshed');
...
AlgodexApi.prototype = {
  ...
  setConfig(config, options = {throws: true, validate: true}) {
    ...
    if (!_.isEqual(config, this.config)) {
      ...
      this.http = {
        ...
        tiny: new TinymanClient(config.tinyman.uri),
      };
  ...
  setAddresses(
    ...
    // console.log('Setting Addresses', _addresses);
    const err = _getSetterError(
        _addresses,
        {
          key: 'Addresses',
