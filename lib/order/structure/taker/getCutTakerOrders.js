 ...
 * @return {Promise<Txns>}
 */
async function handleCloseOutForExecuteAssetTxns(order) {
  return await makeExecuteAssetTxns(order, order.shouldClose);
}
/**
 *
 * @param {*} order
 * @return {Promise<Txns>}
 */
async function handleCloseOutForExecuteAlgoTxns(order) {
    ...
    for (let jj = 0; jj < splitTimes; jj++) {
      ...
      const singleOrderTransList =
                !queuedOrder.isASAEscrow ?
                    await handleCloseOutForExecuteAlgoTxns(withQueuedOrder(order, queuedOrder)) :
                    await handleCloseOutForExecuteAssetTxns(withQueuedOrder(order, queuedOrder));
