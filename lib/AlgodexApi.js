...
let ASA_ESCROW_ORDER_BOOK_ID = -1;
 ...
 async function waitForConfirmation(txId) {
    return await require('./functions/indexer').waitForConfirmation(txId)
}

async function getAccountInfo(accountAddr) {
    return await require('./functions/indexer').getAccountInfo(accountAddr)
}

function allSettled(promises){
    return require('./functions/indexer').allSettled(promises)
}

async function signWalletConnectTransactions(algodClient, outerTxns, params, walletConnector) {
    return await require('./functions/signing').signWalletConnectTransactions(algodClient, outerTxns, params, walletConnector)
}

async function signMyAlgo(algodClient, outerTxns) {
    return await require('./functions/signing').signMyAlgo(algodClient, outerTxns)
}

async function initSmartContracts(environment) {
    console.warn("Running initSmartContracts from API!!, **** find place in lib'")
...
async function getMinWalletBalance (accountInfo, includesFullAccountInfo = false) {
    if (!includesFullAccountInfo) {
        try {
            accountInfo = await getAccountInfo(accountInfo.address); // get full account info
        } catch (e) {
            return 1000000;
        }
    }
...
function dumpVar(x) {
    console.warn("Running dumpVar from API!!, **** create place in lib'")
    return dexInternal.dumpVar(x);
}

function getNumeratorAndDenominatorFromPrice(limitPrice) {
    console.warn("Running propogateTransactions from API!!, **** find a place in Lib")
...
function getCutOrderTimes (queuedOrder)  {
    console.warn("Running getCutOrdertimes from API!!, import {getCutOrderTimes} from 'functions/base'")
...
async function executeOrder (algodClient, isSellingASA, assetId, 
    ...
    console.warn("Running executeOrder from API!!, **** find place in Lib")
    ...
    // No need for the above declaration and complicates the new structure
    ...
    if (includeMaker) {
        ...
        if (isSellingASA && leftoverASABalance > 0) {
            ...
            makerTxns = await getPlaceASAToSellASAOrderIntoOrderbook(algodClient, 
            ...
            makerTxns = await getPlaceAlgosToBuyASAOrderIntoOrderbook(algodClient,
    ...
    if(!!walletConnector && walletConnector.connector.connected) {
        const confirmedWalletConnectArr = await signAndSendWalletConnectTransactions(algodClient, allTransList, params, walletConnector);
    ...
    let confirmedTransactions = await allSettled(waitConfirmedPromises);
...
function assignGroups (txns) {
    console.warn("Running assignGroups from API!!, **** make file in in lib '")
...
function finalPriceCheck(algoAmount,asaAmount, limitPrice, isSellingASA) {
    console.warn("Running finalPriceCheck from API!!, **** find place in Lib")
...
function getAlgoandAsaAmounts(txnList) {
    console.warn("Running getAlgoandAsaAmounts from API!!, import {getAlgoandAsaAmounts} from 'functions/base'")
    ...
    async function getPlaceAlgosToBuyASAOrderIntoOrderbook(algodClient, makerWalletAddr, n, d, min, assetId, algoOrderSize, signAndSend, walletConnector) {
        console.warn("Running getPlaceAlgosToBuyASAOrderIntoOrderBook from API!!, **** need to find place in lib")

        console.debug("placeAlgosToBuyASAOrderIntoOrderbook makerWalletAddr, n, d, min, assetId",
            makerWalletAddr, n, d, min, assetId);
        let program = buildDelegateTemplateFromArgs(min, assetId, n, d, makerWalletAddr, false, constants.ESCROW_CONTRACT_VERSION);

        let lsig = await getLsigFromProgramSource(algosdk, algodClient, program, constants.DEBUG_SMART_CONTRACT_SOURCE);
        ...
        let makerAccountInfo = await getAccountInfo(makerWalletAddr);
        ...
        if (signAndSend) {
            return await signMyAlgo(algodClient, outerTxns);
        }
    ...
    async function getPlaceASAToSellASAOrderIntoOrderbook(algodClient, makerWalletAddr, n, d, min, assetId, assetAmount, signAndSend, walletConnector) {
        console.warn("Running getPlaceASAToSellASAOrderIntoOrderbook from API!!, **** find place in lib to host( potentially orderExecution file?")
        ...
        let program = buildDelegateTemplateFromArgs(min, assetId, n, d, makerWalletAddr, true, constants.ESCROW_CONTRACT_VERSION);
...
const AlgodexApi = {
    ...
    doAlert : function doAlert() {
        console.warn("Running doAlert from API!!, will be deprecated soon")
        alert(1);
     ...
     */
	getConstants : () => {
        console.warn("Running getConstants from API!! Will be deprecated soon")
     ...
     * @returns {promises}
     */
    allSettled : deprecate(allSettled, {file: './lib/functions/AlgodexApi'}),
    ...
    initSmartContracts : deprecate(initSmartContracts, {file: './lib/functions/AlgodexApi'}), 
    ...
    getOrderBookId : function(isAlgoEscrowApp) {
        console.warn("Running getOrderBookId from API!! Will be deprecated soon")
        if (isAlgoEscrowApp) {
    ...
    getMinWalletBalance: deprecate(getMinWalletBalance, {file: './lib/functions/AlgodexApi'}),
    ...
    initIndexer : function(environment) {
        console.warn("Running initIndexer from API!!, will be deprecated soon")
    ...
    initAlgodClient : function(environment) {
        console.warn("Running initAlgodClient from API!!, will be deprecated soon")
    ...
    waitForConfirmation: deprecate(waitForConfirmation, {file: './lib/functions/AlgodexApi'}),
    ...
    dumpVar: deprecate(dumpVar, {file: './lib/functions/AlgodexApi'}),
    ...
    getNumeratorAndDenominatorFromPrice : deprecate(getNumeratorAndDenominatorFromPrice, {file: './lib/functions/AlgodexApi'}),
    ...
    createOrderBookEntryObj : function createOrderBookEntryObj (blockChainOrderVal, price, n, d, min, escrowAddr, 
        ...
        console.warn("Running createOrderBookEntryObj from API!!, will be deprecated soon")
    ...
    getCutOrderTimes : deprecate(getCutOrderTimes, {file: './lib/functions/AlgodexApi'}),
    ...
    executeOrder : deprecate(executeOrder, {file: './lib/functions/AlgodexApi'}),
    ...
    closeOrderFromOrderBookEntry : async function closeOrderFromOrderBookEntry(algodClient, escrowAccountAddr, creatorAddr, orderBookEntry, version) {
        console.warn("Running closeOrderFromOrderBookEntry from API!!, will be deprecated soon")
            ...
            let accountInfo = await getAccountInfo(escrowAccountAddr);
            let assetId = null;
            if (accountInfo != null && accountInfo['assets'] != null
            ...
            let escrowSource = buildDelegateTemplateFromArgs(min,assetid,n,d,creatorAddr, isAsaOrder, version);
     ...
     *  @returns {void}
     */
    assignGroups: deprecate(assignGroups, {file: './lib/functions/AlgodexApi'}),
    ...
    finalPriceCheck: deprecate(finalPriceCheck, {file: './lib/functions/AlgodexApi'}),
    ...
    getAlgoandAsaAmounts:  deprecate(getAlgoandAsaAmounts, {file: './lib/functions/AlgodexApi'}),
    ...
    signAndSendWalletConnectTransactions: deprecate(signWalletConnectTransactions, {file: './lib/functions/AlgodexApi'}),
    ...
    signAndSendTransactions : deprecate(signMyAlgo, {file: './lib/functions/AlgodexApi'}),
    ...
    generateOrder : function (makerWalletAddr, n, d, min, assetId, includeMakerAddr) {
        console.warn("Running generateOrder from API!!, will be deprecated soon")
    ...
    executeMarketOrder : 
        async function executeMarketOrder(algodClient, isSellingASA, assetId, 
            userWalletAddr, limitPrice, orderAssetAmount, orderAlgoAmount, allOrderBookOrders, includeMaker, walletConnector) {
                console.warn("Running executeMarketOrder from API!!, will be deprecated soon")
    ...
    getPlaceASAToSellASAOrderIntoOrderbook : deprecate(getPlaceASAToSellASAOrderIntoOrderbook, {file: './lib/functions/AlgodexApi'}),
...
/////////////////////////////////
     ...
     *  @returns {string}
     */
    printTransactionDebug :  deprecate(printTransactionDebug, {file: './lib/functions/AlgodexApi'}),
    /**
     * @deprecated
     * @param min
     * @param assetId
     ...
     *  @returns {string}
     */
    buildDelegateTemplateFromArgs : deprecate(buildDelegateTemplateFromArgs, {file: './lib/functions/AlgodexApi'}),
    /**
     * @deprecated
     * @param algosdk
     * @param algodClient
     ...
     *  @returns {string}
     */
    getLsigFromProgramSource : deprecate(getLsigFromProgramSource, {file: './lib/functions/AlgodexApi'}),
    /**
     * @deprecated
     * @param accountAddr
     *  @returns {object}
     */
    getAccountInfo : deprecate(getAccountInfo, {file: './lib/functions/AlgodexApi'}),
