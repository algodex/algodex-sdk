...
function createOrder({price, amount, type, orderTemplate}) {
  ...
  return {
    ...
    amount: amount,
    total: price*amount,
    appId: type === 'sell'? 22045503 : 22045522,
...
describe('test newStructure works with compile step', () => {
  ...
  it.skip('Should handle maker buyOrders', async () => {
    ...
    const buyResult = await withMakerTakerTxns(compiledBuyOrder);
  ...
  it.skip('Should handle taker sellOrders', async () => { // TODO: change the names for all these
    const orderbook = await getOrderbook(assetId, api);
    const makerSellOrder = createOrder({price: 300, amount: 0.001, type: 'sell'});
    ...
    const sellResult = await withMakerTakerTxns(compiledSellOrder);
