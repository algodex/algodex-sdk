...
describe('makeCloseAlgo', () => {
  ...
  it('should have a valid application index', async () => {
    await expect(
        makeCloseAlgoTxns({client: config.client, appId: () => {}}),
    ).rejects.toBeInstanceOf(TypeError);
  });

  it.skip(`should create CloseAlgoTxns`, async () => {
    // Fetch orders
    if (typeof orderbook === 'undefined') {
      const res = await config.api.http.dexd.fetchOrders(
          'wallet',
          config.openAccount.addr,
      );
      // TODO add clean api endpoint
      orderbook = res
          .filter((o) => o.type === 'buy')
          .map(async (o) => {
            return await sdkSigner(
                [
                  await withCloseAlgoTxns(
