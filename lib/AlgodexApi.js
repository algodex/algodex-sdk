...
AlgodexApi.prototype = {
  ...
  setWallet(_wallet, options = {validate: true, merge: false}) {
    ...
    if (wallet.type === 'sdk' &&
      ...
      typeof wallet.sk === 'undefined'
    ) {
      wallet.sk = algosdk.mnemonicToSecretKey(wallet.mnemonic).sk;
      wallet.sign = (outerTxns) => {
        ...
        const signedTransactions = flatGroups.map((txn) => {
          return txn.lsig != null?
            ...
            algosdk.signTransaction(txn.unsignedTxn, wallet.sk);
        });
