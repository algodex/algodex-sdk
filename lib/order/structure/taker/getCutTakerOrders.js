...
async function getCutTakerOrders(api, order, queuedOrders, takerOrderBalance) {
  ...
  for (let i = 0; i < queuedOrders.length; i++) {
    ...
    for (let jj = 0; jj < splitTimes; jj++) {
      ...
      const compiledEscrowWithTxns =
                !queuedOrder.isASAEscrow ?
                    await withExecuteAlgoTxns(withTakerBalanceAmounts(withUserWallet(order.wallet, compiledEscrow), updatedTakerOrderBalance), queuedOrder?.forceShouldClose) :
                    await withExecuteAssetTxns(withTakerBalanceAmounts(withUserWallet(order.wallet, compiledEscrow), updatedTakerOrderBalance), queuedOrder?.forceShouldClose);
