...
const toAlgoUnits = require('./utils/units/toAlgoUnits');
const toAssetUnits = require('./utils/units/toAssetUnits');
 ...
 * @return {object}
 */
function _getNumeratorAndDenominatorFromPrice(limitPrice) {
  ...
  const d = 10**origDecCount * limitPrice;
...
AlgodexApi.prototype = {
  ...
  setAddresses(
      addresses,
      options = {throws: false, merge: false, validate: true},
  ) {
    const err = _getSetterError(
        addresses,
        {
  ...
  async placeOrder({order: _order, wallet: _wallet, orderbook}) {
    ...
    function withNumAndDen(o) {
      // Extract Price Parts
      const price = toAssetUnits(order.price, order.asset.decimals);
      ...
      return {
        ...o,
        price,
        amount: toAlgoUnits(order.amount, order.asset.decimals),
        total: toAssetUnits(order.total),
        N, D,
      };
    }
