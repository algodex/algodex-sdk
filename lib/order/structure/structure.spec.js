...
describe('structure', () => {
    ...
    let signedTxns;
    it('should structure a single transaction when escrow already exists', async () => {
        const order = {
            'asset': {
                'id': 15322902,
                'decimals': 6,
    ...
    it('should cancel an existing buyOrder if it exists', async () => {
        const orderbook = await getOrderbook(15322902, config.api)
        const userEscrow = orderbook.filter(order => {

            return order.orderCreatorAddr === config.openAccount.addr &&
                order.escrowOrderType === 'buy'

        })
        expect(userEscrow).not.toBeTruthy()
