...
describe('structure', () => {
  ...
  it.skip('should structure and send taker split order buy transactions', async () => {
    ...
    const executableOrders = orders.contract.txns.map((execObj) => execObj.contract.txns.map((txn) => txn)); // don't need extra mapping
    ...
    for (const group of signedTransactions) {
      ...
      await algosdk.waitForConfirmation(config.client, txId, 10).do();
    }
  }, 10000);
