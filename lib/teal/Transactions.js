...
async function ApplicationCreateTxn(client, creatorAccount, isAlgoEscrowApp = true, useBlankSource = false) {
    ...
    let approvalProgram = await compileProgram(client, approvalProgramSource);
    let clearProgram = await compileProgram(client, clearProgramSource);
...
async function ExecuteAlgoOrder(algodClient, executorAccount, makerAccount, algoAmountReceiving, asaAmountSending,
    ...
    let transaction3 = await AssetTransferTxn(algodClient, takerAddr, orderCreatorAddr, asaAmountSending, assetId, false);
...
async function ExecuteAssetOrder(algodClient, executorAccount, makerAccount, algoAmountSending, asaAmountReceiving, price, assetId, appId, shouldClose = false) {
    ...
    let transaction2 = await PaymentTxn(algodClient, takerAddr, orderCreatorAddr, algoAmountSending, false);
...
async function PlaceAlgoOrder(algodClient, makerAccount, algoOrderSize, price, assetId, appId, isExistingEscrow = false,
    ...
    let txn = await PaymentTxn(algodClient, makerAddr, lsig.address(), algoOrderSize, false);
    let outerTxns = [];
    ...
    if (!skipASAOptIn) {
        // asset opt-in transfer
        let assetOptInTxn = await AssetTransferTxn(algodClient, makerAddr, makerAddr, 0, assetId, false);
...
async function PlaceAssetOrder(algodClient, makerAccount, asaOrderSize, price, assetId, appId, isExistingEscrow = false) {
    ...
    let assetSendTrans = await AssetTransferTxn(algodClient, makerAddr, lsig.address(), asaOrderSize, assetId,
        false);

    let payTxn = await PaymentTxn(algodClient, makerAddr, lsig.address(), constants.MIN_ASA_ESCROW_BALANCE,
        false);
