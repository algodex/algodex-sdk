...
const makeTakerAssetTxns = require('./makeTakerAssetTxns');
...
describe.skip('makePlaceAssetTxns', ()=>{
  ...
  it('should have a valid client', async ()=>{
    await expect(makeTakerAssetTxns({})).rejects.toBeInstanceOf(AlgodError);
  });

  it('should have a valid application index', async ()=>{
    await expect(makeTakerAssetTxns({client: config.client, appId: ()=>{}})).rejects.toBeInstanceOf(TypeError);
  });

  it('should have a valid contract entry', async ()=>{
    await expect(makeTakerAssetTxns({client: config.client, appId: 123456, contract: {entry: 1234}})).rejects.toBeInstanceOf(TypeError);
  });
  ...
  it('should create PlaceAssetTxns', async ()=>{
    ...
    const outerTxns = await makeTakerAssetTxns(_order, true, false);
