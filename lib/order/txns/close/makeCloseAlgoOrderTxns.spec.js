...
describe('makeCloseAlgo', ()=>{
  ...
  it(`should create CloseAlgoOrderTxns`, async ()=>{
    ...
    if (typeof orderbook === 'undefined') {
      const res = await config.api.http.dexd.fetchWalletOrders(config.openAccount.addr);
      // TODO add clean api endpoint
      orderbook = config.api.http.dexd.mapToAllEscrowOrders({
        buy: res.buyASAOrdersInEscrow,
        sell: res.sellASAOrdersInEscrow,
      })
          .map((o)=> config.api.http.dexd.mapOrderbookToOrders({...o, client: config.client}))
          .filter((o)=>o.type === 'buy')
          .map(async (o)=> {
