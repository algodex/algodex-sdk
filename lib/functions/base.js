...
async function closeOrderFromOrderBookEntry(algodClient, escrowAccountAddr, creatorAddr, orderBookEntry, version) {
    ...
    let lsig = await getLsigFromProgramSource(algosdk, algodClient, escrowSource, constants.DEBUG_SMART_CONTRACT_SOURCE);
    ...
    if (assetId == null) {
        ...
        await closeOrder(algodClient, escrowAccountAddr, creatorAddr, ALGO_ESCROW_ORDER_BOOK_ID, appArgs, lsig, infoForMetadata);
    } else {
        console.debug("closing ASA order");
        await closeASAOrder(algodClient, escrowAccountAddr, creatorAddr, ASA_ESCROW_ORDER_BOOK_ID, appArgs, lsig, assetId, infoForMetadata);
    }
}
...
async function getPlaceAlgosToBuyASAOrderIntoOrderbook(algodClient, makerWalletAddr, n, d, min, assetId, algoOrderSize, signAndSend, walletConnector) {
    ...
    let generatedOrderEntry = generateOrder(makerWalletAddr, n, d, min, assetId);
...
async function getPlaceASAToSellASAOrderIntoOrderbook(algodClient, makerWalletAddr, n, d, min, assetId, assetAmount, signAndSend, walletConnector) {
    ...
    let logSigTrans = await createTransactionFromLogicSig(algodClient, lsig,
...
const AlgodexInternalApi = {
     ...
     * @deprecated
     */
    dumpVar: deprecate(dumpVar, {file: 'base.js'}),
    /**
     * @deprecated
     */
