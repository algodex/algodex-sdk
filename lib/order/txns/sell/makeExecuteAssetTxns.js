...
async function makeExecuteAssetTxns(
  ...
  // TODO use optIn
  const _optedIn = await getOptedIn(order.client, {address: order.address}, order.asset.id); // why are we passing the address when the method expects an object?
  ...
  _outerTxns.push({
    ...
    //         amount: order.contract.total, // represents what used to amountSending
    //     },
