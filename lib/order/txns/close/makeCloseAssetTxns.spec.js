...
describe('makeCloseAsset', () => {
  ...
  it('should have a valid application index', async () => {
    await expect(
        makeCloseAssetTxns({client: config.client, appId: () => {}}),
    ).rejects.toBeInstanceOf(TypeError);
  });

  it.skip(`should create CloseAssetTxns`, async () => {
    ...
    // Fetch the Orderbook
    const orderbook = (
      await config.api.http.dexd.fetchOrders('wallet', config.openAccount.addr)
    ...
    const allTxns = await Promise.all(
        orderbook.map(async (o) => {
          return [
            await withCloseAssetTxns(
                await withLogicSigAccount({
                  ...o,
    ...
    signedTxns = await Promise.all(
        allTxns.map(async (o) => await sdkSigner(o, config.openAccount.sk)),
    );
  }, 50000);
  ...
  if (process.env.TEST_ENV === 'integration') {
    it('should send transactions', async () => {
      ...
      await Promise.all(
          signedTxns.map(async (o) => {
            await config.client
                .sendRawTransaction(o[0].map((txn) => txn.blob))
                .do();
          }),
      );
    });
