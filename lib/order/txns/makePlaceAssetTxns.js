...
async function makePlaceAssetTxns(
  ...
  // TODO use optIn
  let makerAlreadyOptedIntoASA = !optIn ? true : await isOptedIn(order.client, order.address, order.asset.id) //Can we assume if the maker is selling an asa they are opted in?
	let lsigAlreadyOptedIntoAsa = await isOptedIn(order.client, order.contract.lsig.address(), order.asset.id) //DISCUSSION: this query implies the existence of an escrow
