...
const GenerateTransactions = {
  ...
  getAssetSendTxn: async function(
      client,
      fromAcct,
      toAcct,
      amount,
  ...
  getCreateAppTxn: async function(
      client,
      creatorAccount,
    ...
    const approvalProgram = await this.compileProgram(
        client,
        approvalProgramSource,
    );
    const clearProgram = await this.compileProgram(client, clearProgramSource);
  ...
  getCloseAlgoEscrowOrderTxns: async function(
      algodClient,
      creator,
      price,
      assetId,
  ...
  getCloseASAEscrowOrderTxns: async function(
      algodClient,
      creator,
      price,
      assetId,
    ...
    const txn4 = algosdk.makePaymentTxnWithSuggestedParams(
        creatorAddr,
        creatorAddr,
        0,
        undefined,
  ...
  getExecuteAlgoEscrowOrderTxns: async function(
      algodClient,
      executorAccount,
      makerAccount,
      algoAmountReceiving,
    ...
    const orderBookEntry = algodex.generateOrder(
        orderCreatorAddr,
        n,
        d,
        min,
    ...
    const transaction3 = await this.getAssetSendTxn(
        algodClient,
        takerAddr,
        orderCreatorAddr,
        asaAmountSending,
    ...
    if (closeRemainderTo == undefined) {
      ...
      transaction4 = await this.getPayTxn(
          algodClient,
          takerAddr,
          lsig.address(),
          refundFees,
  ...
  getExecuteASAEscrowOrderTxns: async function(
      algodClient,
      executorAccount,
      makerAccount,
      algoAmountSending,
    ...
    const transaction2 = await this.getPayTxn(
        algodClient,
        takerAddr,
        orderCreatorAddr,
        algoAmountSending,
  ...
  getPlaceAlgoEscrowOrderTxns: async function(
      algodClient,
      makerAccount,
      algoOrderSize,
      price,
    ...
    const d = numAndDenom.d;
    console.log(
        'getPlaceAlgoEscrowOrderTxns makerWalletAddr, n, d, min, assetId',
        makerAddr,
        n,
        d,
        min,
    ...
    const program = algodex.buildDelegateTemplateFromArgs(
        min,
        assetId,
        n,
        d,
    ...
    const generatedOrderEntry = algodex.generateOrder(
        makerAddr,
        n,
        d,
        min,
    ...
    const txn = await this.getPayTxn(
        algodClient,
        makerAddr,
        lsig.address(),
        algoOrderSize,
    ...
    if (!skipASAOptIn) {
      // asset opt-in transfer
      const assetOptInTxn = await this.getAssetSendTxn(
          algodClient,
          makerAddr,
          makerAddr,
          0,
  ...
  getPlaceASAEscrowOrderTxns: async function(
      algodClient,
      makerAccount,
      asaOrderSize,
      price,
    ...
    const assetSendTrans = await this.getAssetSendTxn(
        algodClient,
        makerAddr,
        lsig.address(),
        asaOrderSize,
    ...
    const payTxn = await this.getPayTxn(
        algodClient,
        makerAddr,
        lsig.address(),
        constants.MIN_ASA_ESCROW_BALANCE,
        false,
    );
