...
function updateTakerBalance(queuedOrder, takerOrderBalance, isAsaEscrow) {
  if (isAsaEscrow) {
    ...
    // TODO: V1 implementation is confusing and overly complicated
    // Decide on the sanest way to refactor and then test.
    ...
    if (closeoutFromASABalance) {
      takerOrderBalance['closeRemainderTo'] = queuedOrder.escrowCreator;
    }
    ...
    if (takerOrderBalance['algoBalance'] - algoAmountReceiving < 500000) {
      takerOrderBalance['closeRemainderTo'] = queuedOrder.escrowCreator;
    }
