...
const fns = require('../functions/base.js');
const dexInternal = {...fns, ...fns.default};
const algodex = dexInternal;
 ...
 * @todo Create 'close(order)' Abstraction
 ...
 * @todo Convert to Constructor
 * @todo Create 'execute(order)' Abstraction
 * @param {Algodv2} algodClient Algorand SDK Client
...
async function ExecuteAssetOrder(algodClient, executorAccount, makerAccount, algoAmountSending, asaAmountReceiving, price, assetId, appId, shouldClose = false) {
  ...
  const transaction2 = await PaymentTxn(algodClient, takerAddr, orderCreatorAddr, algoAmountSending, false);
  ...
  if (!takerAlreadyOptedIntoASA) {
    transaction2b = await AssetTransferTxn(algodClient, takerAddr, takerAddr, 0, assetId, false);
  }
...
async function PlaceAlgoOrder(algodClient, makerAccount, algoOrderSize, price, assetId, appId, isExistingEscrow = false,
  ...
  const txn = await PaymentTxn(algodClient, makerAddr, lsig.address(), algoOrderSize, false);
  const outerTxns = [];
  ...
  if (!skipASAOptIn) {
    // asset opt-in transfer
    const assetOptInTxn = await AssetTransferTxn(algodClient, makerAddr, makerAddr, 0, assetId, false);
...
async function PlaceAssetOrder(algodClient, makerAccount, asaOrderSize, price, assetId, appId, isExistingEscrow = false) {
  ...
  const assetSendTrans = await AssetTransferTxn(algodClient, makerAddr, lsig.address(), asaOrderSize, assetId,
      false);

  const payTxn = await PaymentTxn(algodClient, makerAddr, lsig.address(), constants.MIN_ASA_ESCROW_BALANCE,
      false);
...
module.exports = {
  /**
     * @deprecated
     */
  compileProgram: deprecate(compileProgram, {...depOpts, throws: false}),
  /**
     * @deprecated
     */
  getLsig: deprecate(getLsig, {...depOpts, throws: false}),
  /**
     * @deprecated
     */
