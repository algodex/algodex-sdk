...
describe('structure', () => {
  ...
  it.skip('should structure and send taker split order buy transactions', async () => {
    ...
    const exeuctableOrders = orders.contract.txns.map((execObj) => execObj.contract.txns.map((txns) => txns.unsignedTxn));

    // TODO: handle mapping to orders
  ...
  it('should prove that two seperately constructed transactions with the same parameters have the same txnId despite being signed seperately', async () => {
