...
const {waitForConfirmation, getAccountInfo, allSettled} = require('./functions/indexer')
const {signWalletConnectTransactions, signMyAlgo} = require('./functions/signing')
...
const AlgodexApi = {
    ...
    allSettled : function(promises) {
        console.warn("Running allSettled from API!!, import {allSettled} from 'functions/indexer'")
    ...
    waitForConfirmation : async function(txId) {
        console.warn("Running waitForConfirmation from API!!, import {waitForConfirmation} from 'functions/indexer'")
    ...
    signAndSendWalletConnectTransactions:
        ...
        console.warn("Running signAndSendWalletConnectTransactions from API!!, import {signWalletConnectTransactions} from 'functions/signing'")
    ...
    signAndSendTransactions :
        async function signAndSendTransactions(algodClient, outerTxns) {
            console.warn("Running signAndSendTransactions from API!!, import {signMyAlgo} from 'functions/signing'")
...
/////////////////////////////////
    ...
    printTransactionDebug : function printTransactionDebug(signedTxns) {
        console.warn("Running printTransactionDebug from API!!, **** need to find a place in lib'")
    ...
    buildDelegateTemplateFromArgs : function buildDelegateTemplateFromArgs(min, assetid, N, D, writerAddr, isASAEscrow, version=3) {
        console.warn("Running buildDelegateTemplateFromArgs from API!! import {buildDelegateTemplateFromArgs} from 'functions/indexer'")
    ...
    getLsigFromProgramSource : async function getLsigFromProgramSource(algosdk, algodClient, program, logProgramSource) {
        console.warn("Running getLsigFromProgramSource from API!!, import {getLsigFromProgramSource} from 'functions/indexer'")
    ...
    getAccountInfo : async function getAccountInfo(accountAddr) {
        console.warn("Running getAccountInfo from API!!, import {getAccountInfo} from 'functions/indexer'")
        // dexInternal doesn't have getAccountInfo
...
module.exports = {
    ...
    signWalletConnectTransactions:signWalletConnectTransactions,
    signMyAlgo:signMyAlgo,
    default: AlgodexApi
