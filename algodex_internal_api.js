...
const AlgodexInternalApi = {
  ...
  initSmartContracts: function initSmartContracts(
    algoOrderBookId,
    asaOrderBookId
  ) {
  ...
  // call application
  createTransactionFromLogicSig: async function createTransactionFromLogicSig(
    client,
    lsig,
    AppID,
    appArgs,
  ...
  getExecuteOrderTransactionsAsTakerFromOrderEntry:
    async function getExecuteOrderTransactionsAsTakerFromOrderEntry(
      algodClient,
      ...
      logger.log(
        "looking at another orderbook entry to execute orderBookEscrowEntry: " +
      ...
      if (!isASAEscrow) {
        ...
        return await this.getExecuteAlgoOrderTxnsAsTaker(
          orderBookEscrowEntry,
          algodClient,
          lsig,
          takerCombOrderBalance,
        ...
        return await this.getExecuteASAOrderTxns(
          orderBookEscrowEntry,
          algodClient,
          lsig,
          takerCombOrderBalance,
  ...
  getExecuteASAOrderTakerTxnAmounts(
    ...
    if (
      algoTradeAmount.compareTo(new BigN(maxTradeAmount)) == GREATER_THAN &&
      algoTradeAmount.compareTo(bDecOne) == GREATER_THAN &&
      algoTradeAmount.subtract(new BigN(maxTradeAmount)).compareTo(bDecOne) ==
        GREATER_THAN
    ) {
      logger.log(
        "here999a reducing algoTradeAmount, currently at: " +
          algoTradeAmount.getValue()
      )
      ...
      if (escrowAsaTradeAmount.compareTo(bDecOne) == LESS_THAN) {
        //don't allow 0 value
        escrowAsaTradeAmount = bDecOne
      }
      logger.log(
        "here999b reduced to algoTradeAmount escrowAsaAmount",
        algoTradeAmount.getValue(),
        escrowAsaTradeAmount.getValue()
      )
      ...
      if (escrowAsaTradeAmount.getValue().includes(".")) {
        ...
        if (algoTradeAmount.getValue().includes(".")) {
          algoTradeAmount = algoTradeAmount.floor().add(bDecOne) //round up to give seller more money
          logger.log(
            "here999bc increased algo to algoTradeAmount escrowAsaAmount",
            algoTradeAmount.getValue(),
            escrowAsaTradeAmount.getValue()
          )
        }
        logger.log(
          "here999c changed to algoTradeAmount escrowAsaAmount",
          algoTradeAmount.getValue(),
          escrowAsaTradeAmount.getValue()
        )
      }
    ...
    if (
      new BigN(currentEscrowASABalance)
        .subtract(escrowAsaTradeAmount)
        .compareTo(new BigN(min_asa_balance)) == GREATER_THAN
    ) {
      logger.log(
        "asa escrow here9992 (currentASABalance - escrowAsaAmount) > min_asa_balance",
        currentEscrowASABalance,
        escrowAsaTradeAmount.getValue(),
        min_asa_balance
      )
      closeoutFromASABalance = false
    ...
    if (
      takerCombOrderBalance["walletAlgoBalance"] <
      executionFees + parseInt(algoTradeAmount.getValue())
    ) {
      logger.log(
        "here9992b algo balance too low, returning early! ",
        executionFees,
        algoTradeAmount.getValue(),
        takerCombOrderBalance
      )
    ...
    if (escrowAsaTradeAmount <= 0) {
      logger.log(
        "here77zz escrowAsaTradeAmount is at 0 or below. returning early! nothing to do"
      )
      return emptyReturnVal
    }
    if (algoTradeAmount <= 0) {
      logger.log(
        "here77zb algoTradeAmount is at 0 or below. returning early! nothing to do"
      )
      return emptyReturnVal
    }
    ...
    logger.log(
      "almost final amounts algoTradeAmount escrowAsaAmount ",
      algoTradeAmount,
  ...
  getExecuteASAOrderTxns: async function getExecuteASAOrderTxns(
    orderBookEscrowEntry,
    algodClient,
    lsig,
    takerCombOrderBalance,
    ...
    try {
      ...
      } = this.getExecuteASAOrderTakerTxnAmounts(
        takerCombOrderBalance,
        orderBookEscrowEntry
      )
      ...
      let transaction3 = algosdk.makeAssetTransferTxnWithSuggestedParams(
        lsig.address(),
        takerAddr,
        closeRemainderTo,
        undefined,
  ...
  getExecuteAlgoOrderTakerTxnAmounts(
    ...
    logger.log(
      "orderBookEscrowEntry, takerCombOrderBalance",
      this.dumpVar(orderBookEscrowEntry),
      this.dumpVar(takerCombOrderBalance)
    )
    ...
    if (
      ...
      logger.log(
        "here6a2 figuring out adjusted price for hasSpecialCaseGoodPrice",
    ...
    if (
      ...
      if (algoAmountReceiving.getValue().includes(".")) {
        ...
        logger.log(
          "here6aa decreasing algoAmount due to decimal: " +
            algoAmountReceiving.getValue()
        )
      }
      if (
        new BigN(currentEscrowAlgoBalance).compareTo(algoAmountReceiving) ==
        LESS_THAN
      ) {
        algoAmountReceiving = new BigN(currentEscrowAlgoBalance)
      }

      algoAmountReceiving = algoAmountReceiving.subtract(
        new BigN(0.002 * 1000000)
      ) // reduce for fees
      ...
      logger.log(
        "recalculating receiving amount to: " + algoAmountReceiving.getValue()
      )
    }

    if (
      new BigN(takerCombOrderBalance["asaBalance"]).compareTo(asaAmount) ==
      LESS_THAN
    ) {
      logger.log("here8")
      logger.log(
        "here8 reducing asa amount due to taker balance: ",
        asaAmount.getValue()
      )
      ...
      if (algoAmountReceiving.getValue().includes(".")) {
        ...
        algoAmountReceiving = algoAmountReceiving.floor()
        logger.log(
          "here10 increasing algoAmount due to decimal: " +
            algoAmountReceiving.getValue()
        )
      }
    }
  ...
  getExecuteAlgoOrderTxnsAsTaker: async function getExecuteAlgoOrderTxnsAsTaker(
    orderBookEscrowEntry,
    algodClient,
    lsig,
    takerCombOrderBalance,
    ...
    try {
      ...
      logger.log(
        "orderBookEscrowEntry, algodClient, takerCombOrderBalance",
        this.dumpVar(orderBookEscrowEntry),
        algodClient,
        takerCombOrderBalance
      )
      ...
      const { algoAmountReceiving, asaAmountSending, txnFee } =
        this.getExecuteAlgoOrderTakerTxnAmounts(
          orderBookEscrowEntry,
          takerCombOrderBalance
        )
      ...
      if (
        currentEscrowAlgoBalance - algoAmountReceiving <
        constants.MIN_ESCROW_BALANCE
      ) {
        closeRemainderTo = orderCreatorAddr
      }
      if (orderBookEscrowEntry.useForceShouldCloseOrNot) {
  ...
  getQueuedTakerOrders: function getQueuedTakerOrders(
    takerWalletAddr,
    isSellingASA_AsTakerOrder,
    allOrderBookOrders
  ) {
    logger.log(
      "getQueuedTakerOrders order book list isSellingASA_AsTakerOrder: " +
        isSellingASA_AsTakerOrder
    )
    ...
    for (let i = 0; i < allOrderBookOrders.length; i++) {
      ...
      if (
        orderBookEntry["escrowOrderType"] == "buy" &&
        !isSellingASA_AsTakerOrder
      ) {
      ...
      if (
        orderBookEntry["escrowOrderType"] == "sell" &&
        isSellingASA_AsTakerOrder
      ) {
  ...
  closeASAOrder: async function closeASAOrder(
    algodClient,
    escrowAddr,
    creatorAddr,
    index,
    ...
    try {
      ...
      // Make payment tx signed with lsig
      let txn3 = algosdk.makePaymentTxnWithSuggestedParams(
        lsig.address(),
        creatorAddr,
        0,
        creatorAddr,
  ...
  getAccountInfo: async function getAccountInfo(accountAddr) {
    try {
      ...
      const response = await axios.get(
        ALGOD_SERVER + port + "/v2/accounts/" + accountAddr,
  ...
  // close order
  closeOrder: async function closeOrder(
    algodClient,
    escrowAddr,
    creatorAddr,
    appIndex,
  ...
  printTransactionDebug: function printTransactionDebug(signedTxns) {
    ...
    if (
      constants.DEBUG_SMART_CONTRACT_SOURCE == 1 &&
      constants.INFO_SERVER != ""
    ) {
      ;(async () => {
        try {
  ...
  buildDelegateTemplateFromArgs: function buildDelegateTemplateFromArgs(
    ...
    logger.log(
      "here913 in buildDelegateTemplateFromArgs. min, assetid, N, D, writerAddr, isASAEscrow, orderbookId, version",
      min,
      assetid,
      N,
      D,
