...
async function makePlaceAlgoTxns(
  ...
  if (typeof order?.wallet?.assets === 'undefined') {
    ...
    ({accountInfo} = await order.indexer.lookupAccountById(order.address).do()); // We need to have the same structure between the conditionals
  } else {
    accountInfo = order.wallet;
  }

  const _optIn = await getOptedIn(order.indexer, accountInfo, order.asset.id);
