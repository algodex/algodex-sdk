...
const getQueuedTakerOrders = require('./getQueuedTakerOrders');
const getTakerOrderInformation = require('./getTakerOrderInformation');
const getCutTakerOrders = require('./withCutTakerTxns');
...
async function getTakerOrders(api, order) {
   ...
   * @type {boolean}
   */
  const isMultiOrderExecution = _order.total > firstOrderBalance;

  /**
   * Flag for if the User's order has Executable Orders
   * @type {boolean}
   */
  const isExecutable = _order.type === 'buy' ?
  ...
  if (isMultiOrderExecution) {
    ...
    // TODO: refactor to getCutTakerOrders
    return await getCutTakerOrders(await getTakerOrderInformation(_order, _queuedOrders));
  }
}
