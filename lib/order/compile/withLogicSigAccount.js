 ...
 * @private
 */
function _getTemplateError({min, assetid, N, D, orderBookId}) {
  if (typeof min !== 'number') {
    return new TypeError('Invalid minimum value!');
  }
  if (typeof assetid !== 'number') {
    return new TypeError('Invalid Asset Id');
  }
  if (typeof N !== 'number') {
    return new TypeError('Invalid Numerator');
  }
  if (typeof D !== 'number') {
    return new TypeError('Invalid Denominator');
  }
  if (typeof orderBookId !== 'number') {
    return new TypeError('Invalid Orderbook ID');
  }
}
/**
...
function _compileDelegateTemplate(order = {}) {
  ...
  // Check properties
  const err = _getTemplateError({min, assetid, N, D, orderBookId});
