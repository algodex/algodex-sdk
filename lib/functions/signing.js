...
if (MyAlgo != null) {
   ...
   async function propogateTransactions(algodClient, outerTxns){
     ...
     * @returns {Promise<*>}
     */
    async function signMyAlgoTransactions (outerTxns) {
    ...
    async function signWalletConnectTransactions (algodClient, outerTxns, params, walletConnector) {
            ...
            let resultsFormatted = result.map((element, idx) => {
                return element ? {
            ...
            let orderedRawTransactions = resultsFormatted.map(obj => obj.blob);
...
module.exports = {
    propogateTransactions: propogateTransactions,
    signMyAlgoTransactions: signMyAlgoTransactions,
