...
const groupBy = (items, key) =>
  items.reduce(
      (result, item) => ({
        ...result,
        [item[key]]: [...(result[item[key]] || []), item.signedTxn.blob],
      }),
      {},
  );
...
describe('makePlaceAlgoTxns', () => {
  ...
  it('should have a valid application index', async () => {
    await expect(
        makePlaceAlgoTxns({client: config.client, appId: () => {}}),
  ...
  let signedTxns;
  it.skip('should create PlaceAlgoTxns', async () => {
    const order = {
    ...
    outerTxns.forEach((inner) => {
      ...
      expect(['pay', 'appl', 'axfer'].includes(inner.unsignedTxn.type)).toEqual(
          true,
      );
      ...
      if (inner.unsignedTxn.type === 'appl') {
        ...
        expect(dec.decode(inner.unsignedTxn.appArgs[1])).toEqual(
            _order.contract.entry.slice(59),
        );
      }
